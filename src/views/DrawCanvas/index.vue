...
const formatTimelineActivityText = (item: SessionOperationItemVO) => {
  ...
  const op = formatSessionOperationTypeLabel(item.operationType)
  return `${actor} ${op} ${item.objectKey || '对象'}`
}
...
const formatConflictLogSummary = (item: SessionConflictLogItemVO) => {
  ...
  const type = formatConflictTypeLabel(item.conflictType)
  return `${type}：字段 ${field} 已按策略 ${formatConflictResolveStrategyLabel(item.resolveStrategy)} 处理。`
}
